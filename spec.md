...
- Store editable BMC state in structured `business_model_blocks` for MCP/ChatGPT querying; the heavy tldraw whiteboard experiment is removed from the user flow. BMC and project UI writes create audit/status rows so external MCP changes can be surfaced by lightweight polling.
